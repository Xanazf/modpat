/**
 * MapperWorkspace - per-call workspace for Mapper.perceive (was Resolver.resolveSequence).
 *
 * The Mapper's perception engine used to live in Resolver. That engine held
...
/** A pre-allocated workspace for one in-flight Mapper.perceive call. */
export class MapperWorkspace {
...
/** Backward-compat alias - tests and other modules may still reference `ResolverSlot`. */
export const ResolverSlot = MapperWorkspace;
export type ResolverSlot = MapperWorkspace;

/**
 * Fixed-capacity pool of MapperWorkspaces backed by a RingBuffer free-list.
 *
 ...
 */
export class MapperWorkspacePool {
  private readonly free: RingBuffer<MapperWorkspace>;
  private readonly waiters: Array<(slot: MapperWorkspace) => void> = [];
  private readonly capacity: number;

  constructor(capacity: number, maxN: number) {
    this.capacity = capacity;
    this.free = new RingBuffer<MapperWorkspace>(capacity);
    for (let i = 0; i < capacity; i++) {
      this.free.add(new MapperWorkspace(maxN));
    }
  }
   ...
   */
  acquire(): Promise<MapperWorkspace> {
    if (!this.free.isEmpty()) {
      return Promise.resolve(this.free.removeFirst());
    }
    return new Promise<MapperWorkspace>(resolve => this.waiters.push(resolve));
  }

  /** Return a workspace to the pool, handing it to the next waiter if any. */
  release(slot: MapperWorkspace): void {
    slot.reset();
...
export const ResolverSlotPool = MapperWorkspacePool;
export type ResolverSlotPool = MapperWorkspacePool;
