...
export class GridAtomizer extends BaseAtomizer {
   ...
   */
  public findShapes(frame: number[][][]): Array<{
    id: number;
    color: number;
    x: number;
    y: number;
  ...
  public atomizeShape(
    ...
    const initialScope = this.getSymbolScope(
      `shape_${shape.color}_${shape.width}x${shape.height}`,
      false
    );
